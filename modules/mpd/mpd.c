...
static void init_db()
{
	if (pz_get_int_setting(config, UPDATE_DB) == 1) {
		...
		while (!(kill(pid, 0)));
		pz_set_int_setting(config, UPDATE_DB, 0);
...
static void init()
{
	...
	if (!pz_get_setting(config, UPDATE_DB))
		pz_set_int_setting(config, UPDATE_DB, 1);
	pz_menu_add_setting_group("/Music/Enable MPD", "~Settings", ENABLE_MPD, config, off_on_options);
	
	if (pz_get_int_setting(config, ENABLE_MPD) == 1) {	
		pz_menu_add_setting_group("/Music/Update next startup", "~Settings", UPDATE_DB, config, off_on_options);
